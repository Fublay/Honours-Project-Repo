    ...
    def set_axis_coverage(self, axis_statuses: list[dict]):
        ...
        if not axis_statuses:
            ...
            for status in axis_statuses:
                ...
                state = "complete" if bool(status.get("bootstrap_complete", status.get("complete", False))) else "in progress"
                lines.append(
                    f"{axis_name:<2}  {distinct} / {target} {bar(ratio)} {state}"
                )
                lines.append(f"     span {span:.4f} / {span_target:.4f}")
            message = "\n".join(lines)
